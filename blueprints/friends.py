...
@login_required
def users():
    sida,sub_menu=common_route('Add Friends',['/friends/sendMsg','/users','/friends/viewMsg'],['Send Message','Add Friend','View Messages'])
    all_users = User.query.filter(User.id != current_user.id).all()  # Exkludera den inloggade användaren
    return render_template('/friends/users.html', users=all_users)
...
@login_required
def friends():
    sida,sub_menu=common_route('Friends',['/friends/sendMsg','friends/users','/friends/viewMsg'],['Send Message','Add Friend','View Messages'])
